...
@Injectable()
export class MemberRefreshTokenJwtAuthGuard extends AuthGuard('member-refresh-token-jwt')
{
    constructor(@Inject(CACHE_MANAGER) private cacheManager: Cache) { super() }
    ...
    async canActivate(context: ExecutionContext): Promise<boolean>
    {
        ...
        function action(canActiveResult: boolean): Promise<boolean>
        {
            if (canActiveResult)
            {
                ...
                return Promise.resolve(request.user && request.user[ 'id' ] && !blockedAccounts.includes(request.user[ 'id' ]))
            }
