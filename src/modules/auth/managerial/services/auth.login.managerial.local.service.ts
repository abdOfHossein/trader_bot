...
import { LoginManagerialDataRequestDTO } from '../dtos/auth/login/login.managerial.data.request.dto'
...
export class AuthLoginManagerialLocalService
{
    ...
    async validateUser({ username, password }: LoginManagerialDataRequestDTO)
    {
        ...
        if (account && await bcrypt.compare(password, account[ 'authenticationData' ].password))
        {
            return {
